...
LOAD CSV WITH HEADERS FROM "file:///import/products.csv" AS row
CREATE (n:Product)
SET n = row,
...
LOAD CSV WITH HEADERS FROM "file:///import/categories.csv" AS row
CREATE (n:Category)
SET n = row;

LOAD CSV WITH HEADERS FROM "file:///import/suppliers.csv" AS row
CREATE (n:Supplier)
SET n = row;
